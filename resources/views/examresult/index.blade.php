<x-app-layout>
    ...
    <div class="mx-auto px-6">
        ...
        <div class="sm:hidden">
            <x-h3>学校の試験</x-h3>
            @foreach($grouped_examresults as $examresults)
                @if(Auth::user()->grade == "保護者")
                    <div class="mt-6">
                        <x-h4>{{$examresults[0]->name}}</x-h4>
                    </div>
                @endif
                @foreach($examresults as $examresult)
                    <div class="bg-sky-100 mb-4 p-2">
                        <p class="font-bold">{{$examresult->exam_date}}：{{$examresult->exam_name}}<p>
                        <div class="ml-4">
            ...
            <x-h3>模試</x-h3>
            @foreach($grouped_moshiresults as $moshiresults)
                @if(Auth::user()->grade == "保護者")
                    <div class="mt-6">
                        <x-h4>{{$moshiresults[0]->name}}</x-h4>
                    </div>
                @endif
                @foreach($moshiresults as $moshiresult)
                    <div class="bg-sky-100 mb-4 p-2">
                        <p class="font-bold">{{$moshiresult->exam_date}}：{{$moshiresult->exam_name}}<p>
                        <div class="ml-4">
